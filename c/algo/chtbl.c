...
int
chtlb_init(CHTbl *htbl, int buckets, int (*h)(const void *key),
...
chtbl_lookup(const CHTbl *htbl, void **data){
    ...
    int             bucket;
    bucket = htbl->h(*data) % htbl->buckets;
    for (element = list_head(&htbl->table[bucket]);element != NULL; element = list(element)){
        if (htbl->match(*data, list_data(element))){
