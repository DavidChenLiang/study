...
/*
 *	remote execute client with a loop
 */
...
main()
{
	...
	printf("connected to remote server.  let's see some requests.\n");

	/*
	 *	loop until the user types eof or something like that
	 */
	...
	while ( printf("command? "), fflush(stdout), gets(cmd) )
	{
		if ( write( sock_id, cmd, strlen(cmd) ) == -1 )
			oops( "write" );		
		printf("about to read stuff from remote process..\n");
